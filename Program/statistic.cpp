...
{
	showTitleAnalyzingBooks(); // in Header giao diện
	...
	LLNodeBook lsbook;
	Init(lsbook); // Khởi tạo danh sách liên kết
...
void analyzingCatoBook(){
	showTitleAnalyzingCato(); // In Header giao diện
	FILE *fileBook = fopen(_DIR_DATA_FOLDER_BOOK, "rb");
	if (fileBook == NULL)
	{
		return;
	...
	Init(lsCate); // Tạo danh sách liên kết
...
void runningAnalyzing(int typeAccount){
	switch(typeAccount){
		case 1: // admin
			...
			break;
		case 2: // chuyên viên - expert
			runningAnalyzingForExpert();
			break;
		default: // quản lý - manager
